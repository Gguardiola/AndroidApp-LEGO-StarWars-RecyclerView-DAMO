...
public class MainActivity extends AppCompatActivity {
    ...
    public void insertData(String name, String type, int age, String planet, String affiliations, byte[] image) {
    ...
    private void updateData(int id, String name, String type,int age, String planet, String aff, byte[] selectedImage) {
        ContentValues values = new ContentValues();
        values.put("name", name);
        values.put("type", type);
        values.put("age", age);
    ...
    public void showData() {
        ...
        while (!c.isAfterLast())
        {
            ...
            characterDAMO cDAMO = new characterDAMO(c.getInt(0), c.getString(1), c.getString(2), c.getInt(3), c.getString(4), c.getString(5), c.getBlob(6));
            lchars.add(cDAMO);
            c.moveToNext();
        }
        c.close();
    ...
    private ArrayList<characterDAMO> getCharactersData() {
        Cursor c = db.rawQuery("SELECT * FROM characters", null);
        if(c.getCount() == 0) {
            insertData("Boba Fett", "Bountyhunter",34, "Kamino","Hutt Clan", drawableToBLOB(R.drawable.boba));
            insertData("Clone Trooper", "Republic soldier",5, "Coruscant","Grand Army of the Republic", drawableToBLOB(R.drawable.clone));
            insertData("Jawa", "Alien",23,"Tatooine","Jawa's tribe", drawableToBLOB(R.drawable.jawa));
            insertData("Krell", "Jedi",56,"Coruscant","Jedi Order",drawableToBLOB(R.drawable.krell));
            insertData("Obi-wan", "Jedi",26,"Coruscant","Jedi Order",drawableToBLOB(R.drawable.obi));
            insertData("Yoda", "Jedi",504,"Coruscant","Jedi Order",drawableToBLOB(R.drawable.yoda));
            insertData("Qui-gon", "Jedi",65,"Coruscant","Jedi Order",drawableToBLOB(R.drawable.quigon));
            insertData("Darth Maul", "Lord Sith",21,"Dathomir","Nightbrothers",drawableToBLOB(R.drawable.maul));
            insertData("Akbar", "Rebel Grand Admiral",43,"Mon Cala","Moncalamari",drawableToBLOB(R.drawable.akbar));
            insertData( "Darth Vader", "Lord Sith",41,"Mustafar","Empire",drawableToBLOB(R.drawable.vader));
        }
        ArrayList<characterDAMO> lchars = new ArrayList<characterDAMO>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getDataBase("legoStarWars_19.db");
                    ...
                    public void onActivityResult(ActivityResult result) {
                        if(result != null && result.getResultCode() == RESULT_OK){
                            if(result.getData() != null){
                                ...
                                String age = result.getData().getStringExtra("age");
                                String planet = result.getData().getStringExtra("planet");
                                String aff = result.getData().getStringExtra("affiliations");
                                insertData(name, type,Integer.parseInt(age), planet, aff, selectedImage);
                                lChars = getCharactersData();
                                adapter.setItems(lChars);
                                adapter.notifyDataSetChanged();
                    ...
                    public void onActivityResult(ActivityResult result) {
                        if(result != null && result.getResultCode() == RESULT_OK){
                            if(result.getData() != null){
                                ...
                                String planet = result.getData().getStringExtra("planet");
                                String aff = result.getData().getStringExtra("affiliations");
                                updateData(id, name, type, Integer.parseInt(age), planet, aff, selectedImage);
                                lChars = getCharactersData();
                                adapter.setItems(lChars);
                                adapter.notifyDataSetChanged();
                                lChars = getCharactersData();
        ...
        editBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                intent.putExtra("age", String.valueOf((lChars.get(adapter.getSelectedItem())).getAge()));
